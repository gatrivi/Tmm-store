/**
 * Dedicated storefront for /demo/carniceria — matches visual target mockup.
 * Content source of truth remains docs/roadmap/demo-carniceria-gabriel.md
 */
...
function priceLabel(item: MenuItemType): string {
  ...
  if (kilo) return `${formatArs(kilo.price)}/kg`;
  const only = item.options[0];
  return only ? formatArs(only.price) : '';
}

function isWeightItem(item: MenuItemType): boolean {
  return item.options.some(o => o.id === 'half' || o.id === 'kilo');
}

export default function CarniceriaDemoPage() {
  ...
  const cutsRef = useRef<HTMLElement>(null);
  ...
  if (!demo || !theme || !copy) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#F4EFE7] text-[#1D1B18]">
        Demo carnicería no configurada.
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <section className="relative isolate overflow-hidden">
        <img
          src={demo.heroImage || '/demos/carniceria/gabriel-hero.jpg'}
          alt=""
          className="absolute inset-0 h-full w-full object-cover"
          style={{ objectPosition: demo.heroObjectPosition || 'center 40%' }}
        />
        <div className="absolute inset-0 bg-gradient-to-r from-[#1D1B18]/78 via-[#1D1B18]/45 to-transparent" />
      ...
      <div className="border-b border-black/6 bg-[#ebe4d8]/70">
        <div className="mx-auto grid max-w-6xl gap-3 px-4 py-3 text-xs font-bold text-black/60 sm:grid-cols-3 sm:px-6">
          <p className="flex items-center gap-2">
            <Clock3 size={15} style={{ color: theme.salvia }} /> Peso aproximado
          </p>
          <p className="flex items-center gap-2">
            <ShieldCheck size={15} style={{ color: theme.salvia }} /> Total confirmado antes de preparar
          </p>
          <p className="flex items-center gap-2">
      ...
      {/* Products */}
      <main ref={cutsRef} className="mx-auto max-w-6xl scroll-mt-28 px-4 py-8 sm:px-6 sm:py-10">
              ...
              >
                ...
                <div className="flex flex-1 flex-col gap-3 p-4">
                  ...
                  <p className="text-[10px] font-medium text-black/40">
                    {copy.totalLabel} · Gabriel confirma antes de preparar.
                  </p>
                </div>
              </article>
            );
      ...
      <footer className="border-t border-black/8 bg-[#fffdf9]">
        <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:grid-cols-3 sm:px-6">
          {[
            ...
            { icon: ShoppingBag, title: 'Retiro coordinado', body: copy.pickupHint },
        ...
        <div className="fixed inset-0 z-40 flex justify-end bg-black/45">
          <div className="flex h-full w-full max-w-md flex-col bg-[#fffdf9] shadow-2xl">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              {cart.length === 0 ? (
                <p className="text-sm text-black/45">Todavía no agregaste cortes.</p>
              ) : (
                cart.map((line, idx) => (
