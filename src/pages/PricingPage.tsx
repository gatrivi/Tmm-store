...
const plans = [
  ...
  },
  {
    ...
    features: [
      ...
      'Panel simple + carga inicial hasta 300 productos',
    ],
  },
  {
    name: 'Premium',
...
const comparisonRows = [
  ...
  { label: 'Abono mensual obligatorio', basic: 'No', standard: 'No', premium: 'No', emphasis: true },
...
export default function PricingPage() {
  const contactHref = buildSalesContactHref('pricing — quiero elegir un plan');

  useEffect(() => {
    document.title = 'Planes y precios — Gatrivi.com';
    document
      .querySelector('meta[name="description"]')
      ?.setAttribute(
        'content',
        'Compará los planes Básico, Estándar y Premium de Gatrivi.com. Pagás la implementación sin abono mensual obligatorio.',
      );
  }, []);
      ...
      <main>
        <section className="border-b border-black/10 bg-black py-16 text-white sm:py-24">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-[#ff8a5c]">Planes y precios</p>
            ...
            <div className="mt-7 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-black text-white/85">
              <ShieldCheck size={16} className="text-[#ff8a5c]" />
              Sin abono mensual obligatorio
            </div>
          </div>
        </section>
        ...
        <section className="border-b border-black/10 py-14 sm:py-20">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
              ...
              {plans.map(plan => (
                ...
                >
                  ...
                  <div className="mt-6 border-y border-black/10 py-5">
                    {plan.promo ? (
                      <>
                        <p className="text-xs font-black uppercase tracking-[0.12em] text-[#d84d1d]">Promo lanzamiento</p>
                      ...
                      <>
                        <p className="text-xs font-black uppercase tracking-[0.12em] text-black/40">Implementación</p>
                        <p className="mt-2 text-4xl font-black tracking-[-0.05em]">{plan.full}</p>
                      </>
                    )}
                    <p className="mt-3 text-xs font-bold text-black/45">Pago del proyecto · sin cuota mensual obligatoria</p>
                  </div>
            ...
            <Link
              to="/soporte"
              className="group mt-6 flex items-center justify-between gap-5 rounded-[1.5rem] border border-black/10 bg-white px-5 py-5 transition hover:border-black/25 sm:px-7"
            >
              <div>
                <p className="text-xs font-black uppercase tracking-[0.12em] text-black/35">Opcional después de publicar</p>
                <p className="mt-1 text-base font-black">¿Querés llamarnos por cualquier cambio y que nos encarguemos?</p>
              </div>
              <span className="shrink-0 text-sm font-black text-[#d84d1d]">
                Ver soporte <ArrowRight size={16} className="ml-1 inline transition group-hover:translate-x-1" />
              </span>
            </Link>
          </div>
        </section>
        ...
        <section className="border-b border-black/10 py-14 sm:py-20">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-4 text-4xl font-black tracking-[-0.055em] sm:text-5xl">Proyecto primero. Soporte después, si lo querés.</h2>
            </div>

            <div className="mt-9 grid gap-4 md:grid-cols-2">
              <article className="rounded-[2rem] border border-black/10 bg-white p-6 sm:p-8">
                <p className="text-xs font-black uppercase tracking-[0.13em] text-[#d84d1d]">Para empezar</p>
                <h3 className="mt-4 text-2xl font-black tracking-[-0.035em]">Reservás con $65.000.</h3>
                <p className="mt-3 text-sm font-medium leading-relaxed text-black/60">
                  La seña se descuenta íntegramente del precio acordado. El resto se paga según la propuesta del proyecto. Si necesitás repartir el pago, lo vemos antes de empezar.
                </p>
                <Link
                  to="/reservar?plan=standard"
                  className="mt-6 inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#ff6b35] px-5 text-sm font-black text-white"
                >
                  Reservar con seña
                  <ArrowRight size={16} />
                </Link>
              ...
              <article className="rounded-[2rem] border border-black/10 bg-white p-6 sm:p-8">
                <p className="text-xs font-black uppercase tracking-[0.13em] text-black/35">Después de publicar</p>
                <h3 className="mt-4 text-2xl font-black tracking-[-0.035em]">No quedás atado a una cuota.</h3>
                <p className="mt-3 text-sm font-medium leading-relaxed text-black/60">
                  Podés pedir cambios cuando los necesites y cotizarlos aparte. Si preferís tener prioridad y cambios incluidos todos los meses, existe un servicio de soporte opcional.
                </p>
                <Link
                  to="/soporte"
                  className="mt-6 inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-black/15 px-5 text-sm font-black text-black"
                >
                  Ver soporte opcional
                  <ArrowRight size={16} />
                </Link>
              </article>
            </div>
        ...
        <section className="bg-[#171717] py-14 text-white sm:py-20">
          <div className="mx-auto grid max-w-6xl gap-8 px-4 sm:px-6 lg:grid-cols-[1fr_.8fr] lg:items-center">
            <div>
              ...
              <h2 className="mt-4 text-4xl font-black tracking-[-0.055em]">¿Cuál elijo?</h2>
