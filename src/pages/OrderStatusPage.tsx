...
export default function OrderStatusPage() {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-surface" style={shellStyle}>
        <Loader2 className="animate-spin" style={theme ? { color: theme.bordo } : undefined} size={32} />
      </div>
    );
  }
  ...
  if (!order) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-surface p-6 text-center" style={shellStyle}>
        {vertical && <div className="fixed inset-x-0 top-0"><DemoRibbon /></div>}
        ...
        <Link to={paths.customerPath} className="font-bold hover:underline" style={theme ? { color: theme.bordo } : undefined}>
          Volver al menú
        </Link>
      </div>
    );
  ...
  const statusColor: Record<string, string> = theme
    ? {
        new: 'bg-[#7B1E24]/15 text-[#7B1E24]',
        preparing: 'bg-amber-500/15 text-amber-900',
        ready: 'bg-emerald-500/15 text-emerald-900',
        out_for_delivery: 'bg-emerald-500/15 text-emerald-900',
        completed: 'bg-black/8 text-black/60',
  ...
  const brand = vertical?.siteSettings.brandName;

  return (
    <div className="min-h-screen bg-surface" style={shellStyle}>
      {vertical && <DemoRibbon />}
      <div className="mx-auto max-w-md p-4 pb-10">
        <div
          className="mt-4 overflow-hidden rounded-2xl border shadow-lg"
          style={theme
            ? { backgroundColor: '#fffdf9', borderColor: `${theme.carbon}14` }
            : undefined}
        >
          <div
            className="p-6 text-center text-white"
            style={{ backgroundColor: theme?.bordo ?? '#000' }}
          >
            {brand && (
              <p className="text-[10px] font-black uppercase tracking-[0.14em] text-white/75">{brand}</p>
            )}
            <h1 className="mt-1 text-lg font-black">Estado del pedido</h1>
          ...
          <div className="space-y-4 p-6">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <PaymentBadge order={order} tone={theme ? 'light' : 'dark'} />
            </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-black" style={theme ? { color: theme.bordo } : undefined}>
            ...
            <Link
              ...
              className={
                theme
                  ? 'block w-full rounded-xl py-3 text-center font-bold text-white transition'
                  : 'block w-full rounded-xl bg-green-600 py-3 text-center font-bold text-white transition hover:bg-green-700'
              }
